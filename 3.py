...
def enter_simbol(simbol, pl, memory_vin, step):
    num = int(
        input(f'Игрок номер {pl} определите номер ячейки в которую ставим {simbol} : '))
    while board[num-1] == "X" or board[num-1] == "O":
        print("Эта ячейка уже занята")
        num = int(input(
            f'Игрок номер {pl} еще раз определите номер ячейки в которую ставим {simbol} : '))
    else:
        board[num-1] = simbol
        step += 1
...
memory = [[1, 2, 3], [4, 5, 6], [7, 8, 9], [1, 4, 7],
